<?php
    ...
    if($user->Is_Loggedin() != true)
    {
        ...
        if(isset($_REQUEST['submit-edit'])){
            try{   
                $sentencia = $conn->prepare("UPDATE proyectos SET NOMBRE = :NOMBRE, DESCRIPCION= :DESC,
                                             FECHA_INICIO = :FECHFROM, FECHA_TERMINO = :FECHTO 
                ...
                $sentencia->bindParam(':ID', $_POST['id_proyecto']);
                $sentencia->execute();
        ...
        if(isset($_REQUEST['submit-add'])){
            try{
                $sentencia = $conn->prepare("INSERT INTO proyectos (ID_ORGANIZACION, NOMBRE, DESCRIPCION, FECHA_INICIO, FECHA_TERMINO, ELIMINADO)
                ...
                if($sentencia->execute()){
                    $user->Redirect('proyectos_proyecto.php');
                }  
            }catch(PDOException $e){
                echo 'Fallo la conexion:'.$e->GetMessage();
            }
    ...
    <body>
        <div class="container">
            ...
            <div class="page-header">
                <h1>Proyectos <?php if ($_SESSION['id_rol'] == "1"){echo '
                    <button type="button" class="btn pull-right btn-success" id="add_proyect" name="add_proyect" data-toggle="modal" data-target="#new_proyect">Agregar proyecto<i class="fa fa-edit"></i></button>';}?>
                </h1>
            </div>
            ...
            <!-- Modal -->
            <div id="new_proyect" class='modal fade' role='dialog'>
                <div class='modal-dialog'>
                <!-- Modal content-->
                    <div class='modal-content'>
                        <div class='modal-header'>
